...
#todo diventa una funzione a se stante nome selezione indirizzo
    for prodotto in carrello :
        query = (""" SELECT ricetta FROM FarmaciMagazzino WHERE codice == prodotto["codice_farmaco"] AND ricetta =="si" """)
        serve_ricetta = pd.read_sql_query(query, connection)

        if not serve_ricetta.empty:
            query = (""" SELECT codice FROM FarmaciMagazzino WHERE codice == prodotto["codice_farmaco"] """)
            codicefarmaco= pd.read_sql_query(query, connection)
            codice_fiscale_utente = dati_utente()

            query = (""" SELECT codice_farmaco FROM Ricette WHERE codice_farmaco == codicefarmaco AND codice_fiscale == codice_fiscale_utente""")
